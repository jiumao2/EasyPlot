...
%               d = d12 or [d12 d13 d23]
    ...
    [ax, A0, I0, Z0, nCirc, fminOpts, vennOpts, patchOpts] = parseArgsIn (varargin);
...
function hCirc = drawCircles(hParent, xc, yc, r, P, V)
    ...
    for i = 1:n
        xx = X(i,:)+xc(i);  
        yy = Y(i,:)+yc(i);
        hCirc(i) = patch (hAx, xx, yy, c{i}, 'FaceAlpha', fa{i}, 'Parent', hParent, 'Tag', tag{i});
        if i==1
...
function [ax, A0, I0, Z0, nCircles, fminOpts, vennOpts, patchOpts] = parseArgsIn (args)
    ax = args{1};
    args = args(2:end);
