function violins = violinplot(axes, data, cats, varargin)
...
if not(hascategories) && isvector(data{1})
    violins = EasyPlot.Utils.Violin(axes, data, 1, varargin{:});
    set(axes, 'XTick', 1);
...
elseif ismatrix(data{1})
    ...
    set(axes, 'XTick', 1:size(data{1}, 2));
    if hascategories && length(cats) == size(data{1}, 2)
        set(axes, 'XTickLabels', cats);
    end
end
