...
%BOUNDEDLINE Plot a line with shaded error/confidence bounds. From Kelly Kearney at https://github.com/kakearney/boundedline-pkg
%
...
% 
% ax(1) = subplot(2,2,1);
% [l,p] = EasyPLot.boundedLine(x, y1, e1, '-b*', x, y2, e2, '--ro');
% title('Opaque bounds, with outline');
% 
% ax(2) = subplot(2,2,2);
% EasyPLot.boundedLine(x, [y1;y2], rand(length(y1),2,2)*.5+.5, 'alpha');
% title('Transparent bounds');
% 
% ax(3) = subplot(2,2,3);
% EasyPLot.boundedLine([y1;y2], x, e1(1), 'orientation', 'horiz')
% title('Horizontal bounds');
% 
% ax(4) = subplot(2,2,4);
% EasyPLot.boundedLine(x, repmat(y1, 4,1), permute(0.5:-0.1:0.2, [3 1 2]), ...
...
for ii = 1:length(xl)
    ...
    if strcmp(nanflag, 'fill')
        xp{ii} = EasyPlot.Utils.inpaint_nans(xp{ii}', 4);
        yp{ii} = EasyPlot.Utils.inpaint_nans(yp{ii}', 4);
...
if strcmp(nanflag, 'gap')
    [xp, yp] = EasyPlot.Utils.singlepatch(xp, yp);
else
    xp = xp{1};
    yp = yp{1};
end
