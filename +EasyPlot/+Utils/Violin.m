classdef Violin < handle
    ...
    methods
        function obj = Violin(axes, data, pos, varargin)
            ...
            namefun = st.name;
            args = obj.checkInputs(axes, data, pos, varargin{:});
            ...
            if numel(data) == 1
                obj.MedianPlot = scatter(axes, pos, data, 'filled');
            ...
            switch args.DataStyle
                case 'scatter'
                    if ~isempty(data2)
                        jitter = 1*(rand(size(data))); %right
                        obj.ScatterPlot = ...
                            scatter(axes, pos + jitter.*jitterstrength, data, args.MarkerSize, 'filled');
                        % plot the data points within the violin area
                        ...
                        obj.ScatterPlot2 = ...
                            scatter(axes, pos + jitter.*jitterstrength, data2, args.MarkerSize, 'filled');         
                    else 
                        obj.ScatterPlot = ...
                            scatter(axes, pos + jitter.*jitterstrength, data, args.MarkerSize, 'filled');
                ...
                case 'histogram'
                    ...
                    switch args.HalfViolin
                        case 'right'
                            obj.HistogramPlot= plot(axes, [pos-((counts')/max(counts))*max(jitterstrength)*2, pos*ones(size(counts,2),1)]',...
                                [edges(1:end-1)+max(diff(edges))/2; edges(1:end-1)+max(diff(edges))/2],'-','LineWidth',1, 'Color', 'k');
                        case 'left'
                            obj.HistogramPlot= plot(axes, [pos*ones(size(counts,2),1), pos+((counts')/max(counts))*max(jitterstrength)*2]',...
                                [edges(1:end-1)+max(diff(edges))/2; edges(1:end-1)+max(diff(edges))/2],'-','LineWidth',1, 'Color', 'k');
                        otherwise
                ...
                switch args.HalfViolin
                    case 'right'
                        obj.ViolinPlot =  ... % plot color will be overwritten later
                            fill(axes, [pos+density*width halfViol*pos], ...
                            [value value(end:-1:1)], [1 1 1]);
                    case 'left'
                        obj.ViolinPlot =  ... % plot color will be overwritten later
                            fill(axes, [halfViol*pos pos-density(end:-1:1)*width], ...
                            [value value(end:-1:1)], [1 1 1]);
                    case 'full'
                        obj.ViolinPlot =  ... % plot color will be overwritten later
                            fill(axes, [pos+density*width pos-density(end:-1:1)*width], ...
                            [value value(end:-1:1)], [1 1 1]);
                end
            else
                % plot right half of the violin
                obj.ViolinPlot =  ...
                    fill(axes, [pos+density*width pos-density(1)*width], ...
                    [value value(1)], [1 1 1]);
                % plot left half of the violin
                obj.ViolinPlot2 =  ...
                    fill(axes, [pos-densityC(end)*widthC pos-densityC(end:-1:1)*widthC], ...
            ...
            switch args.QuartileStyle
                ...
                case 'boxplot'
                    obj.BoxPlot = ... % plot color will be overwritten later
                        fill(axes, pos+[-1,1,1,-1]*args.BoxWidth, ...
            ...
            switch args.HalfViolin
                case 'right'
                    obj.MeanPlot = plot(axes, pos+[0,1].*meanDensityWidth, ...
                        [meanValue, meanValue]);
                case 'left'
                    obj.MeanPlot = plot(axes, pos+[-1,0].*meanDensityWidth, ...
                        [meanValue, meanValue]);
                case 'full'
                    obj.MeanPlot = plot(axes, pos+[-1,1].*meanDensityWidth, ...
                        [meanValue, meanValue]);
            end
            ...
            if ~isempty(lowhisker) && ~isempty(hiwhisker)
                obj.WhiskerPlot = plot(axes, [pos pos], [lowhisker hiwhisker]);
            end
                
            % Median
            obj.MedianPlot = scatter(axes, pos, quartiles(2), args.MedianMarkerSize, [1 1 1], 'filled');
                
            % Notches
            obj.NotchPlots = ...
                scatter(axes, pos, quartiles(2)-1.57*IQR/sqrt(length(data)), ...
                [], [1 1 1], 'filled', '^');
            obj.NotchPlots(2) = ...
                scatter(axes, pos, quartiles(2)+1.57*IQR/sqrt(length(data)), ...
                [], [1 1 1], 'filled', 'v');
    ...
    methods (Access=private)
        function results = checkInputs(~, axes, data, pos, varargin)
            isscalarnumber = @(x) (isnumeric(x) & isscalar(x));
            p = inputParser();
            p.addRequired('Axes', @(x)isobject(x));
            ...
            p.parse(axes, data, pos, varargin{:});
            results = p.Results;
        end
    end
